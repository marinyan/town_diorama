...
A browser-based Vite + React + TypeScript diorama centered near Kagurazakaue, scaled wide enough to include the Tokyo Daijingu area. The city is built from procedural Three.js geometry through React Three Fiber and Drei, with tiny retro map icons drifting through alleys, rooftops, stairs, and building entrances.
...
Weather now has a `Live Kagurazaka` mode in the UI. It calls Open-Meteo with coordinates near Kagurazakaue, then maps `weather_code`, `precipitation`, `cloud_cover`, `temperature_2m`, and `wind_speed_10m` back into the same mock ambience state. The current adapter lives in `src/weatherApi.ts`.
...
Time also has a `Now` mode that follows the browser's current clock. A future holiday API can update `isHoliday`, and a location picker can swap the fixed Kagurazaka coordinates for any other city without changing the scene components.

## OSM Map Data

The scene first tries to load `public/data/kagurazaka-osm.json`, which contains OpenStreetMap road and building data fetched through Overpass API. If that file is missing, the app falls back to the deterministic procedural layout.
...
The scene also loads `public/data/kagurazaka-elevation.json` when present. This file is generated from Geospatial Information Authority of Japan elevation tiles and stores a compact relative-height grid for the same Kagurazaka bbox as the OSM sample.
